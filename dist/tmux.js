...
export function splitAndLaunchSidecar(cwd) {
    const sidecarScript = join(__dirname, "sidecar.tsx");
    // Split horizontally (left/right), sidecar takes 40% width on the right
    const paneId = run(`tmux split-window -h -l 40% -P -F '#{pane_id}' -c '${cwd}' 'npx tsx ${sidecarScript} ${cwd}'`);
...
export function launchNewSession(cwd) {
    ...
    run(`tmux split-window -h -l 40% -t ${sessionName} -c '${cwd}' 'npx tsx ${sidecarScript} ${cwd}'`);
    // Focus the left pane
