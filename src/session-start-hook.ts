...
import { splitAndLaunchSidecar, saveSidecarPaneId, saveTrayPaneId, getSidecarPaneId } from "./tmux.js";
...
process.stdin.on("end", async () => {
  ...
  try {
    ...
    // Launch sidecar if not already running
    const existingPane = getSidecarPaneId(cwd);
    let sidecarAlive = false;
    if (existingPane) {
      try {
        execSync(`tmux list-panes -a -F '#{pane_id}' | grep -qF '${existingPane}'`);
        sidecarAlive = true;
      } catch {}
    }
